...
import cashDisbursementResolve from './cash-disbursement-routing-resolve.service';

describe('CashDisbursement routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICashDisbursement returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          cashDisbursementResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(cashDisbursementResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
