...
import { sampleWithRequiredData } from '../cash-disbursement.test-samples';
...
describe('CashDisbursement Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CashDisbursement;
  let fixture: ComponentFixture<CashDisbursement>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CashDisbursement);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 16878 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.cashDisbursements()[0]).toEqual(expect.objectContaining({ id: 16878 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 16878 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.cashDisbursements()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 16878 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to cashDisbursementService', () => {
      const entity = { id: 16878 };
      vitest.spyOn(service, 'getCashDisbursementIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCashDisbursementIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.cashDisbursementsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.cashDisbursementsParams()).toMatchObject(
