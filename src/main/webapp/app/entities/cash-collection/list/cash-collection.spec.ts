...
import { sampleWithRequiredData } from '../cash-collection.test-samples';
...
describe('CashCollection Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CashCollection;
  let fixture: ComponentFixture<CashCollection>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CashCollection);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 8031 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.cashCollections()[0]).toEqual(expect.objectContaining({ id: 8031 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 8031 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.cashCollections()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 8031 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to cashCollectionService', () => {
      const entity = { id: 8031 };
      vitest.spyOn(service, 'getCashCollectionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCashCollectionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.cashCollectionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.cashCollectionsParams()).toMatchObject(
