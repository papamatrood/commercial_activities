...
import { sampleWithRequiredData } from '../debt-payment.test-samples';
import { DebtPaymentService } from '../service/debt-payment.service';

import { DebtPayment } from './debt-payment';

vitest.useFakeTimers();

describe('DebtPayment Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: DebtPayment;
  let fixture: ComponentFixture<DebtPayment>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(DebtPayment);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 942 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.debtPayments()[0]).toEqual(expect.objectContaining({ id: 942 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 942 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.debtPayments()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 942 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to debtPaymentService', () => {
      const entity = { id: 942 };
      vitest.spyOn(service, 'getDebtPaymentIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDebtPaymentIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.debtPaymentsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.debtPaymentsParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
