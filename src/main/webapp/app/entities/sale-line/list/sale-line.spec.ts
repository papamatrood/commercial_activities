...
import { sampleWithRequiredData } from '../sale-line.test-samples';
...
describe('SaleLine Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: SaleLine;
  let fixture: ComponentFixture<SaleLine>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(SaleLine);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 17563 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.saleLines()[0]).toEqual(expect.objectContaining({ id: 17563 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 17563 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.saleLines()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 17563 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to saleLineService', () => {
      const entity = { id: 17563 };
      vitest.spyOn(service, 'getSaleLineIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSaleLineIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.saleLinesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.saleLinesParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
