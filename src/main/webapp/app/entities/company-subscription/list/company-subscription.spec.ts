...
import { sampleWithRequiredData } from '../company-subscription.test-samples';
...
describe('CompanySubscription Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CompanySubscription;
  let fixture: ComponentFixture<CompanySubscription>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CompanySubscription);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 20273 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.companySubscriptions()[0]).toEqual(expect.objectContaining({ id: 20273 }));
  });

  describe('trackId', () => {
    it('should forward to companySubscriptionService', () => {
      const entity = { id: 20273 };
      vitest.spyOn(service, 'getCompanySubscriptionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCompanySubscriptionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.companySubscriptionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
