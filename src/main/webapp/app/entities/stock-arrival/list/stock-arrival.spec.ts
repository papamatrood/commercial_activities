...
import { sampleWithRequiredData } from '../stock-arrival.test-samples';

import { StockArrival } from './stock-arrival';

vitest.useFakeTimers();

describe('StockArrival Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: StockArrival;
  let fixture: ComponentFixture<StockArrival>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(StockArrival);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 5003 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.stockArrivals()[0]).toEqual(expect.objectContaining({ id: 5003 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 5003 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.stockArrivals()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 5003 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to stockArrivalService', () => {
      const entity = { id: 5003 };
      vitest.spyOn(service, 'getStockArrivalIdentifier');
      const id = comp.trackId(entity);
      expect(service.getStockArrivalIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.stockArrivalsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.stockArrivalsParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
